...
function AddItemForm({ onAddItem, resetOnAddItem }) {
  ...
  return (
    ...
    >
      ...
      <label htmlFor="favorite">Favorite Item?</label>
      <br />
      <input
        id="favorite"
        name="favorite"
      ...
      <select
        ...
        style={{ display: "block" }}
      >
        <option value={currentUser.name} label={currentUser.name}>
          {currentUser.name}
        </option>
        {users.map((user) => {
          return (
      ...
      <select
        ...
        style={{ display: "block" }}
      >
        <option value="" label="Select shop">
          Select shop
        </option>
        {shops.map((shop) => {
          return (
      ...
      &nbsp;&nbsp;&nbsp;
      <button type="cancel" onClick={() => resetOnAddItem()}>
        Cancel
      </button>
    </form>
  );
